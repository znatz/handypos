...
- (void)viewDidLoad
{
    ...
    self.selectedBumon = [[NSUserDefaults standardUserDefaults] objectForKey:@"selectedBumon"];
    NSLog(@" ************** %@", self.selectedBumon);
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if([self.settings.picmode intValue]==0){
        ...
        if([self.selectedBumon intValue]==0){//部門管理を使わない場合
    ...
    if([self.selectedBumon intValue]>0) {
        [DataModels select:idArry selectBumon:self.selectedBumon selectFlag:@"1"];
        [DataModels select:titleArry selectBumon:self.selectedBumon selectFlag:@"2"];//タイトルフィールド取り出し
        [DataModels select:PriceArry selectBumon:self.selectedBumon selectFlag:@"3"];
    }
    else{
